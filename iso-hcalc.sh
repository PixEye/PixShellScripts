...
log=~/tmp/schedule.tsv	# Fichier de log (colonnes séparées par des tabulations)
usage="Usage: $cmd -h"		# Message d'aide :
usage=$usage"\n\tAffiche ce message d'aide.\n"

usage=$usage"\nUsage: $cmd disp[lay]|edit"
usage=$usage"\n\tAffiche ou édite directement le fichier de log."
...
echo -n $count|grep -q ^[1-9] || exit 0	# Résultat inférieur à zéro => sortie

#hcount=`n2h $count`			# pas nécessaire en fait
